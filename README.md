...
where the option `--user $(id -u):$(id -g)` gives the user permissions on the outputs of the program; while the option `-v $PWD:/home/docker` mounts the volume of the current working directory into the container, so that python script Higgs.py and the dataset training.csv can be accessed; finally, js `<image_id>` must be replaced with the image identifier.
...
The Git repository contains the following files:
    ...
    - the `Dockerfile`, which contains the instruction to build the container;
